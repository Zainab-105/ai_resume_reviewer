...
export default async function ReviewPage({
  ...
  const exportable: ExportableReview = {
    fileName: resume?.file_name ?? "Resume",
    version: resume?.version ?? null,
    createdAt: review.created_at,
    targetRole: null,
    overallScore: review.overall_score,
    atsScore: review.ats_score,
    subScores,
    atsBreakdown: (review.ats_breakdown ?? []) as AtsCheck[],
    strengths,
    weaknesses,
    suggestions,
    redFlags,
    ...
    <div className="mx-auto w-full max-w-4xl px-4 py-10">
      <div className="mb-6 flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex items-center gap-3">
          <ExportActions
            markdown={reviewToMarkdown(exportable)}
            fileName={exportFileName(exportable, "md")}
          />
          <Link
      ...
      {/* Only on paper — a printed report should say where it came from. */}
      <footer className="mt-8 hidden border-t border-border pt-3 text-xs text-muted-foreground print:block">
        Generated by AI Resume Reviewer. The ATS score is computed from a published rubric; the
        written feedback is AI-generated and quotes the resume as evidence.
      </footer>
    </div>
  );
}
